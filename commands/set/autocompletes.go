...
func (command *Command) requestSetList(s *discordgo.Session, i *discordgo.InteractionCreate, query string, lg discordgo.Locale) {
	if len(strings.TrimSpace(query)) == 0 {
		return
	}

	msg := mappers.MapSetListRequest(query, lg)
	err := command.requestManager.Request(s, i, setRequestRoutingKey, msg, command.autocompleteSetList)
	if err != nil {
